...
test_wait_thread(int num)
{
	...
	// printf("test_wait_thread: start creating thread %d\n", num);///////////////// testing ////////////////
	/* spin for a random time between 0-1 s */
	spin(rand);
	if (num > 0) {
		assert(interrupts_enabled());
		/* wait on previous thread */
		// printf("test_wait_thread: running: %3d, wait: %3d\n", thread_id(), wait[num-1]);///////////////// testing ////////////////
		// printf("test_wait_thread: in the process of creating thread before thread_wait %d\n", num);///////////////// testing ////////////////
		thread_wait(wait[num-1]);
		// printf("test_wait_thread: in the process of creating thread after thread_wait %d, return value %d\n", num, ret);///////////////// testing ////////////////
		assert(interrupts_enabled());
		spin(rand/10);
		/* id should print in ascending order, from 1-127 */
		unintr_printf("id = %d\n", num);
	}
	// printf("test_wait_thread: finish creating thread %d\n", num);///////////////// testing ////////////////
}
...
test_wait(void)
{
	...
	__sync_fetch_and_add(&done, 1);
	for (i = 0; i < NTHREADS; i++) {
		// printf("\n\ntest_wait(): FOR LOOP RUNNING %3ld TIMES\n", i);///////////////// testing ////////////////
		thread_wait(wait[i]);
	}

	struct mallinfo minfo_end = mallinfo();
	// printf("Memory leak: %d\n", minfo_end.uordblks - minfo_start.uordblks);
	assert(minfo_end.uordblks == minfo_start.uordblks);
	assert(minfo_end.hblks == minfo_start.hblks);
...
test_cv_signal()
{
	...
	for (i = 0; i < NTHREADS; i++) {
		thread_wait(result[i]);
		// printf("test_cv_signal: running: %3d, wait: %3d\n", thread_id(), wait[num-1]);///////////////// testing ////////////////
	}
